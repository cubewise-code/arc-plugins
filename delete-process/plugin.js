...
arc.run(['$rootScope', function ($rootScope) {
   ...
   $rootScope.plugin("cubewiseDeleteTI", "Delete", "menu/process", {
      icon: "fa-trash",
...
arc.service('cubewiseDeleteTI', ['$rootScope', '$tm1', 'ngDialog', '$dialogs', '$http', '$helper', function ($rootScope, $tm1, ngDialog, $dialogs, $http, $helper) {
   ...
   this.execute = function (instance, name) {
      ...
      var deleteObject = function (options) {
         var config = {
            method: "DELETE",
            url: encodeURIComponent(instance) + "/Processes('" + $helper.encodeName(name) + "')"
         };
      ...
      var dialog = ngDialog.open({
         template: "__/plugins/delete-process/template.html",
         data: {
            target: "",
            includeRules: true,
            includeData: true,
